...
import { S as SvgComposer, g as generateBadge, c as convertToWebp, p as partitionTiers, t as tierPresets, v as version, l as loadConfig, r as resolveProviders, a as guessProviders, b as resolveAvatars, d as rasterizeSvg } from './shared/sponsorkit.f05976ba.mjs';
...
async function getRoundedAvatars(sponsor) {
  if (!sponsor.avatarBuffer || sponsor.type === "User")
    return sponsor;
  const highRes = Buffer.from(sponsor.avatarBuffer, "base64");
  const [
    mediumRes,
    lowRes
  ] = await Promise.all([
    convertToWebp(highRes, 80),
    convertToWebp(highRes, 50)
  ]);
  return {
    ...sponsor,
    avatarUrlHighRes: highRes.toString("base64"),
    avatarUrlLowRes: lowRes.toString("base64"),
    avatarUrlMediumRes: mediumRes.toString("base64")
  };
}
